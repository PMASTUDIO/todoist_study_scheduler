try {
  require("electron-reloader")(module);
} catch (_) {}

const path = require("path");
const RegisterTodoistTasks = require("./src/registerTodoistTasks");

const {
  app,
  BrowserWindow,
  Menu,
  MenuItem,
  globalShortcut,
  Tray,
  nativeImage,
  ipcMain,
  dialog,
} = require("electron");

const { nextDay, addDays } = require("date-fns");

let tray;
let activeWindow;
let classesWindow;

const createWindow = () => {
  activeWindow = new BrowserWindow({
    width: 850,
    height: 80,
    frame: false,
    resizable: false,
    alwaysOnTop: true,
    webPreferences: {
      preload: path.join(__dirname, "preload.js"),
    },
  });

  activeWindow.loadFile("static/index.html");
  activeWindow.webContents.openDevTools();
};

const createClassSetupWindow = () => {
  classesWindow = new BrowserWindow({
    width: 400,
    height: 300,
    resizable: false,
    alwaysOnTop: true,
  });

  classesWindow.on("close", () => {
    app.dock.hide();
    classesWindow = null;
  });

  classesWindow.loadFile("static/courses.html");
};

const menu = new Menu();
menu.append(
  new MenuItem({
    label: "PKG Study Scheduler",
    submenu: [
      {
        label: "Close",
        accelerator: "Esc",
        click: () => {
          if (activeWindow) {
            activeWindow.close();
            activeWindow = null;
          }
          if (classesWindow) {
            classesWindow.close();
            classesWindow = null;
          }

          app.dock.hide();
        },
      },
    ],
  })
);

Menu.setApplicationMenu(menu);

app.whenReady().then(() => {
  const icon = nativeImage.createFromDataURL(
    "data:image/png;base64,"
  );
  tray = new Tray(icon.resize({ width: 16 }));

  app.dock.hide();

  const contextMenu = Menu.buildFromTemplate([
    {
      label: "Schedule Study",
      click: () => {
        if (app.isHidden() || BrowserWindow.getAllWindows().length === 0) {
          app.dock.show();
          createWindow();
        }
      },
    },
    {
      label: "Change Classes",
      click: () => {
        if (!classesWindow) {
          app.dock.show();
          createClassSetupWindow();
        }
      },
    },
    {
      label: "Quit",
      click: () => {
        app.quit();
      },
    },
  ]);

  // tray.setToolTip("This is my application.");
  tray.setContextMenu(contextMenu);

  globalShortcut.register("Shift+CommandOrControl+S", () => {
    if (BrowserWindow.getAllWindows().length === 0) {
      app.dock.show();
      createWindow();
    }
  });

  ipcMain.handle(
    "submitReview",
    async (_, classId, content, difficulty, date) => {
      console.log(classId, content, difficulty, date);

      activeWindow.close();
      activeWindow = null;

      app.dock.hide();

      await RegisterTodoistTasks(
        [date, addDays(date, 1), addDays(date, 2), addDays(date, 3)],
        classId,
        content,
        difficulty
      );

      dialog.showMessageBoxSync({
        title: "You're all set!",
        message: `Your study sessions for ${classId} on "${content}" were scheduled in your selected calendar app. Difficulty was set to ${difficulty} and the due date / test date is ${date.toString()}`,
      });
    }
  );
});

app.on("activate", () => {
  if (BrowserWindow.getAllWindows().length === 0) createWindow();
});

app.on("window-all-closed", () => {
  if (process.platform !== "darwin") app.quit();

  app.dock.hide();
});
